      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 items-stretch">
          {services.map((service, index) => (
            <Card
              key={index}
              className="hover-lift border-border bg-card group relative overflow-hidden flex flex-col min-h-[480px] md:min-h-[520px] h-full w-full max-w-[340px] mx-auto"
            >
              {service.highlight && (
              ...
              <CardContent className="flex-1 flex flex-col justify-between">
                <ul className="space-y-3 mb-6">
                  {service.features.map((feature, featureIndex) => (
                    <li
                      key={featureIndex}
                      className="flex items-center space-x-3 font-body text-sm"
                    >
                ...
                <OperatorSelector
                  message={`Olá! Gostaria de solicitar um orçamento para ${service.title}.`}
                >
                  <Button
                    variant="outline"
